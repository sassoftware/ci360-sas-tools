...
data sample_json_data;
   length id 8 name $20 age 8 json_text $200;

   /* Input data delimited by "|" */
   infile datalines dlm='|';

   /* Input variables */
   input id name $ age json_text $;

   /* Output the data */
   datalines;
1|John|25|{"address": "123 Main St", "city": "New York"}
2|Alice|30|{"address": "456 Elm St", "city": "Los Angeles"}
3|Bob|28|{"address": "789 Oak St", "city": "Chicago"}
;
run;

proc ds2; 
  data json_2_cols (overwrite=yes);
  
    * Declare the columns to create from JSON and temp columns *;
    dcl char(200) address city ;
    dcl int rc;
    ...
    * Drop temp column *;
    ...
    method run() ;
      ...
      set sample_json_data ;
      
      /** Start: Extract columns from JSON text **/
      j.createParser( json_text );
      ...
      do while (rc = 0) ;    
        ...
        if (token = 'address') AND j.islabel(tokenType, parseFlags) then do ;
          j.getNextToken( rc, token, tokenType, parseFlags);
          address=token;
        end ;
        else if (token = 'city') AND j.islabel(tokenType, parseFlags) then do ;          
          j.getNextToken( rc, token, tokenType, parseFlags);
          city=token;
        end ;
      ...
      output ;
      j.destroyParser(); 
      /** END: Extract columns from JSON text **/
    end ;
